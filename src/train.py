...
momentum_max = 0.5
momentum_d = 0.9
criterion = nn.CrossEntropyLoss(reduction="mean", label_smoothing=0.01)
...
for epoch in range(args.max_epochs):
    ...
    epoch_losses = []
    if epoch >= 10:
        loss_diff = mean((last_epoch_loss)[-2_000:]) - (previous_model_loss or 999_999)
        if loss_diff <= 0:
    ...
    for batch, (x, c, y) in enumerate(dataloader):
        ...
        disc_real, (d_state_h, d_state_c) = discriminator(
            (y.float(), c), (d_state_h, d_state_c)
        )
        real_labes = torch.zeros((y.shape[0], y.shape[1], 1)).to(device)
        disc_real_loss = mse_loss(disc_real, real_labes)
        ...
        # fake_factor = 1.0
        fake_labels = torch.mean(torch.abs(y_pred - y), dim=-1).view(disc_fake.shape).float()
        disc_fake_loss = mse_loss(disc_fake, fake_labels)
        ...
        real_labes = torch.zeros((y.shape[0], y.shape[1], 1)).to(device)
        loss_d = mse_loss(disc_pred, real_labes) * fake_factor
        ...
        real_error = torch.mean(torch.abs(torch.nn.functional.softmax(y_pred, dim=-1) - y)).item()
        ...
        loss_history_discriminator.append([loss_d.item(), 1])
        ...
        loss_diff1 = mean(([previous_model_loss or 999_999] + last_epoch_loss)[-2_000:])
        loss_diff =  loss_diff1 - (previous_model_loss or 999_999) * (1 + config.save_margin)
        isBetter = loss_diff < 0
        log(
            f"{epoch}:{batch} - loss: {round(log10(loss.item()), 2)} ({round_log10(loss_r.item())} + {round_log10(loss_d.item())}), lr: {round(log10(scheduler.get_last_lr()[-1]), 3)}, better: {isBetter} ({round(round_log10(loss_diff1) - round_log10(previous_model_loss), 2)}), error: {round((real_error) * 100, 2)}%",
...
last_epoch_loss = mean(last_epoch_loss[-2_000:])
...
plot_simple_array(
    [
        [round(log10(x[0]), 2) for x in loss_history_real],
        [round(log10(x[0]), 2) for x in loss_history_discriminator],
