...
class Discriminator(nn.Module):
    def __init__(self, config):
        ...
        self.embedding_linear = nn.Linear(vocab_size, self.embedding_dim)
    ...
    def forward(self, inputs, prev_state):
        ...
        c = self.embedding(c)
        c_shape = c.shape
        c = torch.cat((e.view((x.shape[0], x.shape[1], -1, self.embedding_dim)), c.view((x.shape[0], x.shape[1], -1, self.embedding_dim))), dim=-2)[:, :, :-1]
        c = c.view(c_shape)
